...
class FileBasedMockData {
    ...
    getSourceReference(aggregationAnnotation) {
        ...
        if (!referentialConstraint || referentialConstraint.length === 0) {
            throw new Error(`Referential constraints must be defined for the ParentNavigationProperty: '${parentNavigationProperty.value}'`);
        }
    ...
    getAncestorsOfNode(currentNode, outItems, nodeProperty, hierarchyDefinition, depth, matchedChildrenCount, matchedProperties) {
        ...
        if (currentNode && (depth < 0 || depth > 0)) {
            ...
            if (outItems.includes(currentNode)) {
                ...
                if (hierarchyDefinition.matchedProperty) {
                    (0, common_1.setData)(currentNode, hierarchyDefinition.matchedProperty, !!matchedProperties.find((prop) => {
                        const mainItem = (0, common_1.getData)(prop, nodeProperty);
                        const adjustedItem = (0, common_1.getData)(currentNode, nodeProperty);
    ...
    async getTopLevels(data, _parameters, _odataRequest) {
        ...
        if (aggregationAnnotation) {
            ...
            let adjustedData = data.map((adjustedRowData) => {
                const item = this._mockData.find((dataItem) => {
                    const mainItem = (0, common_1.getData)(dataItem, nodeProperty);
                    const adjustedItem = (0, common_1.getData)(adjustedRowData, nodeProperty);
                    return mainItem === adjustedItem;
                });
            ...
            const restOfData = this._mockData
                ...
                return !data.find((dataItem) => {
                    const mainItem = (0, common_1.getData)(dataItem, nodeProperty);
                    const adjustedItem = (0, common_1.getData)(item, nodeProperty);
                    return mainItem === adjustedItem;
                });
            })
                .map((item) => {
            ...
            const allRootNodes = adjustedData.filter((node) => {
                const parent = adjustedData.find((parent) => {
                    const nodeValue = (0, common_1.getData)(parent, nodeProperty);
                    const sourceReferenceValue = (0, common_1.getData)(node, sourceReference);
            ...
            outItems.forEach((item) => {
                const subTreeData = data.find((dataItem) => {
                    const mainItem = (0, common_1.getData)(dataItem, nodeProperty);
                    const adjustedItem = (0, common_1.getData)(item, nodeProperty);
                    return mainItem === adjustedItem;
                });
    ...
    async getDescendants(inputSet, lastFilterTransformationResult, hierarchyFilter, entityType, _parameters, _odataRequest) {
        ...
        if (aggregationAnnotation) {
            ...
            const adjustedData = this._mockData.map((item) => {
                const adjustedRowData = hierarchyFilter.find((dataItem) => {
                    const mainItem = (0, common_1.getData)(dataItem, nodeProperty);
                    const adjustedItem = (0, common_1.getData)(item, nodeProperty);
                    return mainItem === adjustedItem;
                });
            ...
            hierarchyFilter.forEach((item) => {
                var _a;
                const parentNodeChildren = hierarchyNodes[(0, common_1.getData)(item, sourceReference)];
                if (parentNodeChildren) {
                    const currentNode = parentNodeChildren.find((node) => {
                        const mainItem = (0, common_1.getData)(node, nodeProperty);
                        const adjustedItem = (0, common_1.getData)(item, nodeProperty);
                        return mainItem === adjustedItem;
                    });
            ...
            const outData = [];
            inputSet.forEach((item) => {
                const subTreeData = subTrees.find((dataItem) => {
                    const mainItem = (0, common_1.getData)(dataItem, nodeProperty);
                    const adjustedItem = (0, common_1.getData)(item, nodeProperty);
                    return mainItem === adjustedItem;
                });
    ...
    async getAncestors(inputSet, lastFilterTransformationResult, limitedHierarchy, entityType, _parameters, _odataRequest) {
        ...
        if (aggregationAnnotation) {
            ...
            const sourceReference = this.getSourceReference(aggregationAnnotation);
            const adjustedData = this._mockData.map((item) => {
                const adjustedRowData = limitedHierarchy.find((dataItem) => {
                    const mainItem = (0, common_1.getData)(dataItem, nodeProperty);
                    const adjustedItem = (0, common_1.getData)(item, nodeProperty);
                    return mainItem === adjustedItem;
                });
            ...
            const outData = [];
            inputSet.forEach((item) => {
                const subTreeData = ancestors.find((dataItem) => {
                    const mainItem = (0, common_1.getData)(dataItem, nodeProperty);
                    const adjustedItem = (0, common_1.getData)(item, nodeProperty);
                    return mainItem === adjustedItem;
                });
